...
export const getNextPrayer = (prayers: Prayer[]) => {
  ...
  if (hours > 0 && minutes > 0) {
    return {
      ...nextPrayer,
      remaining: `Осталось ${hours} ${hourText} и ${minutes} ${minuteText}`,
    };
  }

  if (hours > 0 && minutes === 0) {
    const verb = hours === 1 ? 'Остался' : 'Осталось';
    return {
      ...nextPrayer,
  ...
  if (hours === 0 && minutes > 0) {
    const verb = minutes === 1 ? 'Осталась' : 'Осталось';
    return {
      ...nextPrayer,
